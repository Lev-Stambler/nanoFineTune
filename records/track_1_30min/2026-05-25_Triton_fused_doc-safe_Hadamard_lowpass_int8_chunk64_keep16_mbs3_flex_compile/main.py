...
image = (
    ...
    .add_local_file(Path(__file__).with_name("activation_filter_kernels.py"), "/root/activation_filter_kernels.py")
)
...
def run_track1(
    ...
    activation_filter_kernel: Literal["dense", "fwht", "triton-dense"] = "dense",
    ...
    if activation_filter_kernel not in {"dense", "fwht", "triton-dense"}:
        raise ValueError("--activation-filter-kernel must be one of: dense, fwht, triton-dense")
    ...
    if activation_filter_kernel == "triton-dense" and activation_filter_quantization != "int8":
        raise ValueError("--activation-filter-kernel=triton-dense currently requires --activation-filter-quantization=int8")
    ...
    activation_filter_triton_kernels = None
    ...
    def activation_filter_triton_module():
        nonlocal activation_filter_triton_kernels
        if activation_filter_triton_kernels is None:
            import activation_filter_kernels

            activation_filter_triton_kernels = activation_filter_kernels
        return activation_filter_triton_kernels
    ...
    def set_activation_filter_doc_ids(doc_ids: torch.Tensor, index_device: torch.device) -> None:
        ...
        for token_count in (seq_len, seq_len - 1):
            ...
            lowpass_starts_by_chunk_size: dict[int, list[int]] = {
                chunk_size: [] for chunk_size in activation_filter_lowpass_chunk_sizes
            }
            ...
            for row_idx in range(doc_ids_cpu.shape[0]):
                ...
                while start < token_count:
                    ...
                    while cursor < end:
                        ...
                        lowpass_starts_by_chunk_size[chosen_chunk_size].append(chunk_base)
            ...
            for chunk_size in activation_filter_lowpass_chunk_sizes:
                ...
                lowpass_starts = torch.tensor(
                    lowpass_starts_by_chunk_size[chunk_size],
                    device=index_device,
                    dtype=torch.long,
                )
                lowpass_tiers.append(
                    {
                        "chunk_size": chunk_size,
                        "indices": lowpass_tensor,
                        "starts": lowpass_starts,
                    }
                )
    ...
    def triton_dense_int8_encode(
        flattened: torch.Tensor,
        chunk_starts: torch.Tensor,
        chunk_size: int,
        keep: int,
    ) -> tuple[Any, int]:
        basis = activation_filter_basis(flattened.device, flattened.dtype, chunk_size, keep)
        q_values, scale = activation_filter_triton_module().dense_int8_pack(
            flattened,
            chunk_starts,
            basis,
            chunk_size=chunk_size,
        ...
        payload = ("int8", q_values, scale)
        return payload, tensor_nbytes(q_values) + tensor_nbytes(scale)

    def triton_dense_int8_decode_into(
        lowpass: Any,
        chunk_starts: torch.Tensor,
        ...
        if not (isinstance(lowpass, tuple) and len(lowpass) == 3 and lowpass[0] == "int8"):
            return False
        _quantization, q_values, scale = lowpass
        basis = activation_filter_basis(restored.device, restored.dtype, chunk_size, keep)
        activation_filter_triton_module().dense_int8_unpack(
            q_values,
            scale,
            chunk_starts,
            basis,
    ...
    def pack_activation_tensor(tensor: torch.Tensor) -> Any:
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False):
            ...
            for tier in lowpass_tiers:
                ...
                if activation_filter_kernel == "triton-dense":
                    lowpass, tier_lowpass_bytes = triton_dense_int8_encode(
                        flattened,
                        tier["starts"],
                        tier_chunk_size,
                        tier_keep,
    ...
    def unpack_activation_tensor(packed: Any) -> torch.Tensor:
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False):
            ...
            lowpass_tiers_by_chunk_size = {
                int(tier["chunk_size"]): tier for tier in plan["lowpass_tiers"]
            }
            exact_indices = plan["exact_indices"]
            suffix_numel = math.prod(suffix_shape)
            ...
            for tier_chunk_size, tier_keep, lowpass in lowpass_payloads:
                tier = lowpass_tiers_by_chunk_size[tier_chunk_size]
                if activation_filter_kernel == "triton-dense" and triton_dense_int8_decode_into(
                    lowpass,
                    tier["starts"],
                    restored,
                    tier_chunk_size,
