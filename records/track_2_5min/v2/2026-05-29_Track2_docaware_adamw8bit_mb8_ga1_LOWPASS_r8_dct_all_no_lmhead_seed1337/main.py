...
def run_track1(
    ...
    if resolved_optimizer_name == "yaqadamw" and yaqa_mode == "static":
        ...
        _mins = [float(s.min()) for s in yaqa_scales.values()]
        _means = [float(s.mean()) for s in yaqa_scales.values()]
        _maxs = [float(s.max()) for s in yaqa_scales.values()]
        print(f"YAQA calibration complete: {len(yaqa_scales)} layers", flush=True)
        print(f"YAQA scale stats per-layer: min_min={min(_mins):.4f} mean_mean={sum(_means)/len(_means):.4f} max_max={max(_maxs):.4f}", flush=True)
        torch.cuda.synchronize()
